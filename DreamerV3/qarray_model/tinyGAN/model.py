...
class Generator(nn.Module):
    def __init__(self, image_size=128, conv_dim=64, z_dim=128, c_dim=128, repeat_num=5):
        ...
        file_dir = os.path.dirname(__file__)  # Directory of the current script
        weight_path = os.path.join(file_dir, 'cls_weight_reduce.npy')

        weight = torch.FloatTensor(np.load(weight_path))
    ...
    def forward(self, noise, y_emb):
        h = self.fc(noise).view(-1, self.nfilter0, self.W0, self.W0)
        # y_emb = self.embeding(y_emb)
