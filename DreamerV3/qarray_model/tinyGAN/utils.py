...
def restore_model(resume_epoch, ckpt_dir, G, D, g_optimizer=None, d_optimizer=None, load_optimizers=False):
    """Restore the trained models and optionally optimizers."""
    file_dir = os.path.dirname(__file__)
    ckpt_dir = os.path.join(file_dir, ckpt_dir)
    path = os.path.join(ckpt_dir, 'model_{}.tar'.format(resume_epoch))
    checkpoint = torch.load(path, map_location=device)
    
    # Always load generator weights
    ...
    if D:
        ...
        # Only load optimizers if explicitly requested and they exist
        if load_optimizers and g_optimizer is not None:
            try:
                g_optimizer.load_state_dict(checkpoint['g_optimizer'])
                print("Generator optimizer state loaded successfully")
            except (ValueError, KeyError) as e:
                print(f"Could not load generator optimizer state: {e}")
        
        if load_optimizers and d_optimizer is not None:
            try:
                d_optimizer.load_state_dict(checkpoint['d_optimizer'])
                print("Discriminator optimizer state loaded successfully")
            except (ValueError, KeyError) as e:
                print(f"Could not load discriminator optimizer state: {e}")
