...
class GateVoltageComposer:
    ...
    def meshgrid_virtual_coupled(self, dots: List[int | str], arrays: List[np.ndarray], gate_voltages: List[float]) -> np.ndarray:
        """
        Implements meshgrid_virtual such that all dots receive their correct virtual gate voltages for a single scan.
        Useful if we want to capture the full crosstalk effects between non-swept and swept dots.
        """
        assert self.virtual_gate_origin is not None, 'virtual_gate_origin must be set in the init or with model.virtual_gate_origin = ...'
        assert self.virtual_gate_matrix is not None, 'virtual_gate_matrix must be set in the init or with model.virtual_gate_matrix = ...'
        assert self.n_dot is not None, 'n_gate must be set in the init or with model.n_gate = ...'
        assert all([array.ndim == 1 for array in arrays]), 'arrays must be 1d'
        assert len(dots) == len(arrays), 'gates and arrays must be the same length'
        assert self.n_dot == len(gate_voltages), 'gate voltages do not match the number of dots'
        ...
        if len(dots) != 2:
            raise NotImplementedError("meshgrid_virtual_coupled currently only supports a two-dot sweep")
        ...
        if len(gate_voltages) == self.n_dot:
            # gate_voltages missing the sensor dot
            gate_voltages.append(0.0)

        # Set all dots to their gate voltages
        Vd[:] = gate_voltages

        # Override the swept dots using broadcasting
        # sweepleft is x-axis (horizontal, columns, second index)
        # sweepright is y-axis (vertical, rows, first index)
        Vd[:, :, dotleft] = sweepleft[np.newaxis, :]
        Vd[:, :, dotright] = sweepright[:, np.newaxis]
    ...
    def do2d(self, x_gate: str | int, x_min: float, x_max: float, x_res: int, y_gate: str | int, y_min: float,
             y_max: float, y_res: int, gate_voltages: list | None = None, add_full_crosstalk: bool = False) -> np.ndarray:
        """
        ...
        :return:
        """
        if add_full_crosstalk:
            assert gate_voltages is not None, "gate_voltages must be provided to add full crosstalk"
            if isinstance(gate_voltages, np.ndarray):
                gate_voltages = gate_voltages.tolist()

            # check that both gates are virtual (e.g., 'vP1', 'vP2')
            dot_indices = []
            for gate in [x_gate, y_gate]:
                if not isinstance(gate, str) or not patterns['vP'].match(gate):
                    raise ValueError(f'Gate {gate} must be a virtual gate in the form vP[int] when using add_full_crosstalk')
                match = patterns['vP'].match(gate)
                dot_indices.append(int(match.groups()[0]))

            return self._do2d_virtual(dot_indices, [x_min, y_min], [x_max, y_max], [x_res, y_res], gate_voltages)
    ...
    def _do2d_virtual(self, dots: List[int], mins: List[float], maxs: List[float], ress: List[int], gate_voltages: List[float]) -> np.ndarray:
        return self.meshgrid_virtual_coupled(
            dots,
            [np.linspace(mins[i], maxs[i], ress[i]) for i in range(len(dots))],
            gate_voltages
        )