...
class KalmanCapacitanceUpdater:
    """
    Kalman filter for capacitance estimation with NN and NNN support.
    ...
    def update(self, i: int, j: int, delta: float, measurement_variance: float) -> bool:
        ...
        P = self.variances[row, col] + self.process_noise
        x = self.means[row, col]
    ...
    def update_from_scan(
        ...
        if self.include_nnn and len(ml_outputs) == 3:
            ...
            # update() normalizes both to (i, i+1), so Kalman naturally combines
            # them as two independent measurements of the same quantity.
