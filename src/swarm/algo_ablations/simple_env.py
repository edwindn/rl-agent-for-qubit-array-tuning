...
from swarm.algo_ablations.simple_qarray import QarrayBaseClass as DeterministicQarray
...
class QuantumDeviceEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.window_delta = (window_delta_range['min'] + window_delta_range['max']) / 2.0

        radial_noise_config = self.config['simulator']['radial_noise']

        self.array = DeterministicQarray(
        ...
        self.constant_voltage_offset = np.full(
            (self.num_dots,),
            (voltage_offset_range['min'] + voltage_offset_range['max']) / 2.0
        )

        # Add the constant voltage offset to shift the voltage space
    ...
    def _init_voltage_ranges(self, plunger_ground_truths, barrier_ground_truths):
        ...
        plunger_range = (full_plunger_range_width['min'] + full_plunger_range_width['max']) / 2.0

        plunger_center = plunger_ground_truths  # Center on ground truth (middle of allowed range)

        self.plunger_max = plunger_center + 0.5 * plunger_range
        self.plunger_min = plunger_center - 0.5 * plunger_range


        barrier_range = (full_barrier_range_width['min'] + full_barrier_range_width['max']) / 2.0

        barrier_center = barrier_ground_truths  # Center on ground truth (middle of allowed range)
    ...
    def _starting_voltages(self):
        plunger_centers = (self.plunger_min + self.plunger_max) / 2.0

        if self.use_barriers:
            barrier_centers = (self.barrier_min + self.barrier_max) / 2.0
        else:
