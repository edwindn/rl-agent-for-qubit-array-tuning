...
class QarrayBaseClass:
    ...
    def _sample_from_range(self, range_config: dict, rng: np.random.Generator) -> float:
        """Return the middle value from a min-max range configuration."""
        return (range_config["min"] + range_config["max"]) / 2.0
    ...
    def _load_model_no_barriers(self, param_overrides: dict, rng: np.random.Generator = None):
        ...
        """
        if rng is None:
            rng = np.random.default_rng(0)  # Use fixed seed for deterministic behavior
