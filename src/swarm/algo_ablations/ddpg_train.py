...
Focused on the DDPG ablation while keeping the same memory-tracking/logging helpers.
"""
import os
import sys
import warnings
...
from swarm.algo_ablations.DDPG.ddpg import DDPGConfig
...
def load_config(config_path=None, checkpoint_path=None):
    ...
    if checkpoint_path:
        ...
        config_file = Path(__file__).parent / "configs" / "ddpg_training_config.yaml"
    else:
...
def main():
    ...
    try:
        ...
        if algo != "ddpg":
            raise ValueError("The ddpg training script only supports algorithm 'DDPG'. "
                             f"Got '{config['rl_config']['algorithm']}' in the config.")
        ...
        # Remove SAC-specific or legacy params that the DDPG config does not understand.
        for unsupported_param in (
            ...
            training_params.pop(unsupported_param, None)
