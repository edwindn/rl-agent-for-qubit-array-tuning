...
from qarray.qarray_types import (CddInv, Cdd, VectorList, CddNonMaxwell, CgdNonMaxwell, NegativeValuedMatrix)
...
def _convert_to_maxwell_with_barriers_and_sensor(cdd_non_maxwell: CddNonMaxwell, cgd_non_maxwell: CgdNonMaxwell,
                                                cds_non_maxwell: CddNonMaxwell, cgs_non_maxwell: CgdNonMaxwell,
                                                cbd_non_maxwell=None, cbg_non_maxwell=None, 
                                                cbs_non_maxwell=None, cbb_non_maxwell=None):
    """
    Convert non-Maxwell capacitance matrices to Maxwell form, with sensors and barriers.
    
    CORRECTED PHYSICS: Treats barriers as voltage sources only (not charge reservoirs).
    Builds extended system matrices:
    - Cdd: [dots + sensors] × [dots + sensors] (barriers excluded from charge degrees of freedom)
    - Cgd: [dots + sensors] × [gates + barriers] (barriers as voltage sources)
    
    Parameters:
    -----------
    cdd_non_maxwell, cgd_non_maxwell, cds_non_maxwell, cgs_non_maxwell : array-like
        Standard capacitance matrices (same as sensor-only version)
    cbd_non_maxwell : array-like, optional
        Barrier-to-dot capacitance matrix, shape (n_dot, n_barrier)  
    cbg_non_maxwell : array-like, optional
        Barrier-to-gate capacitance matrix, shape (n_barrier, n_gate) - used for cross-coupling only
    cbs_non_maxwell : array-like, optional  
        Barrier-to-sensor capacitance matrix, shape (n_sensor, n_barrier)
    cbb_non_maxwell : array-like, optional
        Barrier-to-barrier capacitance matrix, shape (n_barrier, n_barrier) - not used (barriers don't have charges)
        
    Returns:
    --------
    tuple
        Maxwell converted matrices: (cdd_full, cdd_inv_full, cgd_full)
        Charge system: [dots, sensors] only; Voltage system: [gates, barriers]
    """
    # Extract system dimensions
    n_dot = cdd_non_maxwell.shape[0]
    n_sensor = cds_non_maxwell.shape[0] 
    n_gate = cgd_non_maxwell.shape[1]
    n_barrier = cbd_non_maxwell.shape[1] if cbd_non_maxwell is not None else 0
    
    # Build Cdd matrix: [dots, sensors] × [dots, sensors] (NO barriers in charge space)
    charge_nodes = n_dot + n_sensor
    cdd_non_maxwell_full = np.zeros((charge_nodes, charge_nodes))
    
    # Populate dot-dot block
    cdd_non_maxwell_full[:n_dot, :n_dot] = cdd_non_maxwell
    
    # Populate sensor blocks (same as existing sensor conversion)  
    cdd_non_maxwell_full[n_dot:n_dot+n_sensor, :n_dot] = cds_non_maxwell      # sensor-dot
    cdd_non_maxwell_full[:n_dot, n_dot:n_dot+n_sensor] = cds_non_maxwell.T    # dot-sensor
    
    # Build Cgd matrix: [dots, sensors] × [gates, barriers] (barriers as voltage sources)
    cgd_non_maxwell_full = np.zeros((charge_nodes, n_gate + n_barrier))
    
    # Populate gate coupling blocks (original gates)
    cgd_non_maxwell_full[:n_dot, :n_gate] = cgd_non_maxwell                    # dot-gate
    cgd_non_maxwell_full[n_dot:n_dot+n_sensor, :n_gate] = cgs_non_maxwell     # sensor-gate
    
    # Populate barrier coupling blocks (barriers as voltage sources affecting dots/sensors)
    if n_barrier > 0:
        if cbd_non_maxwell is not None:
            cgd_non_maxwell_full[:n_dot, n_gate:] = cbd_non_maxwell            # dot-barrier voltage coupling
        if cbs_non_maxwell is not None:
            cgd_non_maxwell_full[n_dot:n_dot+n_sensor, n_gate:] = cbs_non_maxwell  # sensor-barrier voltage coupling
    
    # Convert to qarray types and apply Maxwell conversion
    cdd_non_maxwell_full = CddNonMaxwell(cdd_non_maxwell_full)
    cgd_non_maxwell_full = CgdNonMaxwell(cgd_non_maxwell_full)
