"""
BarrierCNNAgent — same Nature-CNN architecture as PlungerCNNAgent but with a
single input channel. GroupedMAC strips the zero-pad channel before calling
this agent's forward (via `native_channels=1`).
"""
...
class BarrierCNNAgent(nn.Module):
    ...
    def __init__(self, obs_shape: tuple[int, int, int], args: Any) -> None:
        ...
        self.args = args
        C, H, W = obs_shape
        assert C == self.native_channels, f"BarrierCNNAgent expects {self.native_channels}-channel input, got {C}"
