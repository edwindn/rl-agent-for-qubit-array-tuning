...
def run(_run, _config, _log, pymongo_client=None):
    ...
    if getattr(args, "use_wandb", False):
        logger.setup_wandb(
            project=getattr(args, "wandb_project", "FACMAC-benchmark"),
            entity=getattr(args, "wandb_entity", None),
            config=dict(_config),
            name=unique_token,
            ema_period=getattr(args, "wandb_ema_period", 20),
        )
    ...
    # After training completes, upload the final saved checkpoint to wandb.
    if getattr(args, "use_wandb", False) and args.save_model:
        final_save_dir = os.path.join(args.local_results_path, "models", args.unique_token)
        if os.path.isdir(final_save_dir):
            step_dirs = sorted(
                [d for d in os.listdir(final_save_dir) if d.isdigit()],
                key=int,
            )
            if step_dirs:
                latest = step_dirs[-1]
                logger.log_artifact(
                    local_dir=os.path.join(final_save_dir, latest),
                    name=f"facmac-agents-{args.unique_token}",
                    metadata={"step": int(latest), "unique_token": args.unique_token, "name": args.name},
                )
...
def run_sequential(args, logger):
    ...
    if 'particle' not in args.env and "cts_matrix_game" not in args.env and "mujoco_multi" not in args.env and "pymarl_quantum" not in args.env:
        ...
        # Extra fields needed by GroupedMAC (pymarl_quantum env).
        if "agent_groups" in env_info:
            args.agent_groups = env_info["agent_groups"]
        if "obs_shapes" in env_info:
            args.group_obs_shapes = env_info["obs_shapes"]
