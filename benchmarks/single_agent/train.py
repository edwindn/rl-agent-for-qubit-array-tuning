...
def load_config_from_path(config_path):
    """Load training configuration from a specific YAML file path."""
...
def main():
    ...
    # Load config from checkpoint if specified, otherwise use default
    checkpoint_config_path = None
    env_config_path = None  # Will be set to checkpoint's env config if loading from checkpoint

    if args.load_checkpoint:
        checkpoint_dir = Path(args.load_checkpoint)
        checkpoint_config_path = checkpoint_dir / "training_config.yaml"
        checkpoint_env_config_path = checkpoint_dir / "env_config.yaml"

        if checkpoint_config_path.exists():
            print(f"\nLoading training config from checkpoint: {checkpoint_config_path}")
            config = load_config_from_path(checkpoint_config_path)
        else:
            print(f"\nWarning: No training_config.yaml found in checkpoint directory {checkpoint_dir}")
            print("Using default training config instead")
            config = load_config()

        if checkpoint_env_config_path.exists():
            print(f"Loading environment config from checkpoint: {checkpoint_env_config_path}")
            env_config_path = str(checkpoint_env_config_path.absolute())
        else:
            print(f"Warning: No env_config.yaml found in checkpoint directory {checkpoint_dir}")
            print("Using default environment config instead")
            # env_config_path stays None, which will use default
    else:
        config = load_config()
        # env_config_path stays None, which will use default
    ...
    try:
        ...
        if args.load_checkpoint:
            ...
            checkpoint_path = Path(args.load_checkpoint).absolute()
            if checkpoint_path.exists():
                print(f"\nLoading checkpoint from: {checkpoint_path}")
                try:
                    # Remove optimizer state from checkpoint unless --keep-optimizer is specified
                    if not args.keep_optimizer:
                        import pickle
                        from ray.rllib.core import COMPONENT_OPTIMIZER

                        learner_state_path = checkpoint_path / "learner_group" / "learner" / "state.pkl"

                        if learner_state_path.exists():
                            print("Removing optimizer state from checkpoint to avoid parameter group mismatch...")
                            print("  (Use --keep-optimizer to preserve optimizer state)")
                        ...
                        print("Keeping optimizer state from checkpoint (--keep-optimizer flag set)")
                    ...
                    if match:
                        ...
                        print(f"Warning: Could not determine iteration number from checkpoint path")
                        print(f"Starting training from iteration 1 with weights from {checkpoint_path}...")
